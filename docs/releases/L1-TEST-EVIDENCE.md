...
| display-safe CLI line approval preserves raw execution | `apollo-code` | `src/runtime.test.ts` — `uses the shared safe formatter for line approval without changing the raw Bash command`; `shows a deny-only marker when sanitization would hide part of a raw Bash command` |
