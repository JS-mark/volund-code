...
| display-safe, secret-safe approval preserves raw execution | `apollo-code` | `src/runtime.test.ts` — `uses the shared safe formatter for line approval without changing the raw Bash command`; `shows a deny-only marker when sanitization would hide part of a raw Bash command`; `redacts a bare ... secret in line display and the permission event` (provider/GitHub/AWS/JWT/Cf table); `enforces deny after a tui handler tries to approve sensitive hidden details`; `fails closed for an approval ... without leaking it to the event` |
