...
| `SD1-01` | SD1 | 数据模型、participant identity、状态 reducer、transition contract | BRAND-VERIFY | Architecture reviewer required |
| `SD1-02` | SD1 | production identity/trust adapter、CAS/fencing/signed anchor/effect recovery | SD1-01 | Storage/security/auth trust owners required |
| `SD1-03` | SD1 | 隔离 worktree、ArtifactRef、manifest/seal | SD1-02 | Security reviewer required |
| `SD2-01` | SD2 | base-SHA policy、可信 TaskClassifier 与 protected surfaces | SD1-03 | Security + repository owner required |
...
| `SD3-03` | SD3 | 独立 Reviewer、AcceptanceReport、shadow eval | SD3-02 + SD1-02 production identity gate | Independent acceptance reviewer required |
| `SD4-01` | SD4 | Human challenge / ApprovalReceipt + post-approver isolation revalidation | SD3-03 + SD1-02 production identity gate | BDFL/product + security UX + independent acceptance re-sign required |
| `SD4-02` | SD4 | 本地 branch-only Promotion Worker | SD4-01 post-approver revalidation gate | Repository owner required |
...
- **Scope**：按 BRAND-VERIFY 关闭的 identity/evidence 新建正式领域包，实现 §18 的 `SelfDevRun`、versioned policy/artifact、TaskClassifier trusted-input/request/result/signed-attestation、Baseline/Candidate/Verification/Acceptance/Approval/Promotion/effect/journal-anchor types、无自引用的 key RegistryPayload/Binding、统一 `PrincipalBinding`/`CredentialBinding`/`ParticipantIdentitySet`、typed reasons、domain-separated canonical encoding 和纯状态 reducer。每个 ActorRef 强制携带 principal/credential binding digest；binding payload 与外层 artifact/digest/signature 分离，issuer 不用 ActorRef 自签。Identity issuer records 只能在 builtin kind↔role↔purpose matrix 内收窄；Human Approver auth payload 使用不含 `authenticationContextDigest` 的 `HumanApproverIdentityRef`，外层 actor 再指向已完成 context。明确区分 existing `baseRef@baseSha` 与事前不存在的 `promotionRef`。Reducer 不 import provider、tools、CLI 或 storage；模型输出永远是 input data，不直接 transition。
- **Dependencies**：BRAND-VERIFY。
- **Tests/evidence**：表驱动逐条覆盖 §18 显式 transition、统一 FAILABLE/CANCELLABLE 集、`STALEABLE` **每个状态**→STALE、reason/state 适用表、清理 guard 和 PROMOTING commit point；特别包含 PROPOSED 的 stale-base/ref-occupied，以及缺失/失配 TaskClassAttestation、ParticipantIdentitySet 时不能跨过对应 guard。`VERIFYING→ACCEPTING` 未冻结 exact-version participant set 必拒绝，freeze 前调用 reviewer、两次 review 之间替换/追加 set 均拒绝。所有未列 transition、未完成 worker/lease/challenge/worktree cleanup、终态恢复、version mismatch、missing guard、inconclusive-as-pass 均拒绝。对 Principal/Credential payload→artifact→binding 做 canonical/domain/no-self-reference golden tests，ActorRef digest 必须与 binding subject/kind/role/purpose 对应；cross-kind/role/purpose issuer scope 拒绝，HumanApproverIdentityRef→context→ActorRef 可按顺序构造且无 digest cycle。加入 property/fuzz test 验证任何状态序列都不能绕过 SEALING/VERIFYING/ACCEPTING/AWAITING_HUMAN，且 cancel/promotion CAS 竞态只收敛到 CANCELLED 或 COMPLETED/FAILED。运行新包 test/typecheck 和 architecture dependency test。
- **Human gate**：独立 architecture reviewer 对照 §18 transition table 逐行签字。

### SD1-02 · Production identity/trust、CAS、monotonic fencing、独立 journal anchor 与 effect recovery

- **Scope**：先建立 **production** identity/trust foundation：在受保护的 storage/auth/launcher 边界实现 trust-root + key-registry adapter，配置并托管 model、service 与 human-run-owner launcher/auth issuer keys，为这些参与者签发 §18 `PrincipalBinding`/`CredentialBinding`，并提供 verification、rotation、revocation、latest-epoch 和 issuer usage/domain/scope/build/validity 查询。Production composition 必须使用 OS/platform 保护的不可导出 key 或经 security owner 批准的独立 protected signer；test/fixture/mock/self-signed adapter 必须是不同实现与配置，不能被 production composition 选择，也不能满足本任务 evidence gate。随后在 `packages/storage` 增加 SelfDev adapter，实现 atomic compare-and-swap、短 lease + 单调 fencing token、corruption-evident hash chain/checkpoint、key RegistryPayload/Binding latest-epoch/revocation 查询、single-use nonce/receipt 和 crash-safe append；在候选/Runner/本地 store 写权限之外提供 protected append-only Ed25519 `JournalAnchorStore` + monotonic counter。Registry verifier 在后续 SD2/SD3 前已能通过同一 production adapter 验证 principal/credential/task-attestation issuer 的 usage/domain/scope/build/validity/revocation；Anchor 只能使用 registry 中 `journal-anchor-issuer` service key/domain，并绑定 service actor/build、latest binding epoch 与 revocation。Effect intent 显式分类为 transactional/queryable、idempotent/retryable 或 opaque/ambiguous，开始前预留 worst-case budget，每个 attempt 计费；只承诺 exactly-once committed state，opaque provider/model/suite 未知结果不盲重放。
- **Dependencies**：SD1-01。
- **Tests/evidence**：先做 production-adapter E2E：由真实 launcher/auth 路径分别为 model、service、human run owner 签发 Principal/Credential bindings，完成 issue→verify→participant use→rotate/revoke→旧 binding/credential 拒绝，并保存受保护 registry/issuer artifact、composition-root、exact-SHA 与 revocation evidence；断言 production profile 无法载入 fixture/mock/static test keys，且 fixture/self-signed 结果不会被 evidence collector 接受。再覆盖并发双 writer、lease expiry/steal/token 单调性，以及 worker 在“authorization 后、commit 前”暂停、被 steal 后恢复写入的 fault injection；断言旧 fence + generation + stateVersion 100% 被拒绝。覆盖 registry issuer usage/domain/scope/build/validity/revocation 查询、latest epoch/previous-binding rollback，以及 journal truncate/reorder/duplicate/unknown schema、**完整链重写**、旧有效 anchor/store snapshot rollback、counter 回退、key revocation、anchor outage、checkpoint rollback；覆盖三类 effect 在 intent/physical attempt/receipt 前后的 crash，验证 transactional reconcile、idempotent retry 逐次计费、opaque 无 receipt 必为 fail/inconclusive 且无 blind replay。运行 storage/auth/launcher integration、composition-root、typecheck 和 Windows/Linux filesystem tests；保存 local bytes、独立 anchor 记录和恢复状态 digest。
- **Human gate**：storage reviewer + security reviewer + auth/trust owner 确认 production key provisioning/custody、issuer scopes、binding issuance、rotation/revocation、production composition、durability、fencing、anchor trust boundary、budget charging 和 fail-closed 语义；fixture/mock/self-signed evidence 不得关闭本门禁，无独立 protected anchor 的实现不得称 tamper-evident/production。
...
### SD2-01 · base-SHA policy、可信 TaskClassifier 与 protected surfaces

- **Scope**：实现 versioned `meet_v1(builtinPolicy,userPolicy,policyLoadedFrom(baseSha))`，把 builtin/user/base source artifacts 与独立 EffectivePolicy artifact 在 PROPOSED 固定。逐字段 lattice 不得自行解释：allowed paths/capabilities/suites 与 baseRef matchers/promotion prefixes 语义取交集，deny 取并集；promotion reservation required 取 OR、backend 精确一致、TTL 取 min；最大 budget/TTL/diff/repair/concurrency 取 min；最低 quality/pass/reviewer count 取 max；required suites 取并集；baseline repair rule 取交集且 new/count/severity regression 永禁；risk path/task/capability trigger 取并集、diff/severity threshold 取更早触发值、normal/high reviewer count 取 max；task-classifier engine/rule version 精确一致，allowed class/static input/per-class baseline-exception set 取交集，forced-high-risk 取并集，static bytes 取 min，fallback reviewer count 取 max；allow-boolean 取 AND。missing/unknown/unfulfillable 全 fail closed。实现由 protected builtin classifier registry/release manifest 固定 expected rule digest、由 `baseSha` policy 固定 constraint digest 的确定性 TaskClassifier，builtin/base 使用不可互换的 typed slots。仅可信 control plane 可从 canonical goal、base path/index/允许的静态输入产生 trusted-input payload→request binding→deterministic `TaskClassificationResult`，再用 registry 中独立 task-attestation issuer key 签 run-scoped `TaskClassAttestation`；caller API 只接收 `taskClassHints`，user/base 只能删除 class/收紧 input/提高 risk。建立 default-deny 路径分类和 store-backed promotionRef reservation。候选 policy/AGENT/Skill/Plugin/CI/模型输出不参与当前 run 的 classification。
- **Dependencies**：SD1-03。
- **Tests/evidence**：对每个 lattice 字段做三源排列、边界、空集、matcher/prefix 交集、deny-vs-allow、reservation race/expiry、baseline rule/authoritative-class 匹配、new/increased/severity regression、risk threshold/trigger/reviewer count、task-class allowed/forced-risk/static-input/per-class-exception/fallback-reviewer meet、required-not-allowed、整数溢出、AND/OR 的 table/property tests；source/effective/classifier artifact bytes/digest 在 run 中改变必拒绝。分类 fixture 必须覆盖 deceptive low-risk label、非空 hint mismatch、unknown/零匹配、mutually-exclusive rules 冲突、classifier/decoder failure、unsupported input：全部得到 high-risk、policy 最大 reviewer、空 authoritative/exception-authorized class set；空 hints 仅表示 no assertion。Mixed eligible+ineligible 多 class fixture 证明 repair rule 只能使用同一个 authorized class，不能串权。User/base 尝试新增 builtin 未产生的 class、放宽 predicate、降低 builtin risk/count 必拒绝。Base policy 缺少 constraint declaration、builtin/user 偷带该字段、PolicyBinding artifact/digest 与 base payload 不同、builtin/base slot swap 全部 schema/runtime 拒绝。Rogue initial builtin artifact、expected digest 不符、合法 signed manifest A + rogue registry payload B 拼接、manifest/binding epoch/version 或 expected rules/groups 不同、epoch rollback，以及 registry/task-attestation cross-domain/wrong-build/expired/revoked issuer 全部拒绝。相同 definition/request/effective policy 跨 run 只要求 `classificationResultDigest` 一致；run/time/signature 不同的 attestation digest 必须可不同。Candidate/model 改标签、diff、policy、文件或换 session 均不能改变当前 result/attestation binding。每一 protected category 至少一个真实路径和新增未知路径 fixture；candidate 修改 policy、symlink/case/rename/Unicode 绕过全部 deny；missing field、unknown field/enum/schema/lattice/canonicalizer、parse/digest error 全 fail closed。输出机器可读 coverage 报告，未分类路径数量必须为 0。
...
- **Scope**：实现 BASELINING/VERIFYING worker、required/safety/holdout suites、deterministic graders、ArtifactRef evidence 和 bundle digest。Trusted suite 输出 versioned failure class/severity/count/output digest；只有 effective baseline rule 同时匹配 suite、failure class 与 `TaskClassificationResult.baselineExceptionAuthorizedTaskClassIds` 中的同一个 authoritative class，才允许目标修复既有失败。Fallback-high-risk、unknown、hint mismatch、conflicting rules 或 classifier failure 的 authorized set 为空；mixed eligible/ineligible class 不能串权。Candidate 不得新增 class、增加 count、提高 severity 或扩大 failure output。Holdout 对 Developer 不可见；反馈只暴露规范化类别。
- **Dependencies**：SD3-01。
- **Tests/evidence**：pass/fail/inconclusive/baseline-existing-failure/timeout/crash/environment drift/holdout leak fixture；逐项覆盖 rule 不匹配、authoritative/authorized class 不匹配、result/attestation digest 不匹配、mixed eligible+ineligible class、fallback/unknown/mismatch/conflict、新 failure class、count+1、severity 升级和 output 扩大，均 fail closed；同输入重复两次 bundle digest 一致；missing/modified evidence、伪造 stdout、candidate 更改 test/config/classification 都不能提升 verdict。记录 trajectory/quality/cost/safety 四类指标。
...
- **Scope**：实现仅允许 model/service `ReviewerActorRef` 的 `selfdev.review` 无工具 profile、新会话/独立 principal、sealed-only 输入和 versioned/length-bounded/strict-schema typed data channel；HumanApprover、run owner、Developer 与其他 role 类型上不可产出 attestation/advisory。所有 Developer、Reviewer、Orchestrator、Verifier、run owner 及其他参与者身份只能由 SD1-02 已通过门禁的 **production identity/trust adapter** 签发、查询和撤销；不得接受 fixture/mock/static ActorRef 或测试 key。受信 identity issuer 校验每个 actor 的 Principal/Credential binding，并由 `VERIFYING→ACCEPTING` transition 原子冻结 performed + designated 的完整 `ParticipantIdentitySet`，覆盖 Developer、run owner、全部 Human Approver、Orchestrator、Verifier、Promotion Worker、Journal Anchor 和 Reviewer；freeze 前不得启动 review。AcceptanceReport/model recommendation 仅 **advisory**；isolation attestation/report 都绑定 TaskClassificationResult、signed TaskClassAttestation、ParticipantIdentitySet、reviewer actor/principal/credential/runtime/profile/prompt/envelope/decoder/fresh session/fresh context-source instance 与 sealed evidence。Deterministic risk classifier 取 task classification risk floor 与 EffectivePolicy triggers/thresholds 的较高风险和 reviewer count。计数 reviewer 的 actor/principal/credential/key/session/context-source 必须两两唯一且不能复用 forbidden participant。完成 SD3 shadow pipeline，但 promotion API 保持不存在。
- **Dependencies**：SD3-02；并显式要求 SD1-02 production identity/trust adapter 的 provisioning、issuance/verification/revocation E2E 与 human gate 已关闭，不能以 transitive dependency 或 fixture 代替。
- **Tests/evidence**：HumanApprover/run-owner/Developer/service 非-reviewer 构造报告全部 type/runtime 拒绝；binding issuer/usage/domain/subject/kind/role/purpose/build/validity/revocation 任一不符拒绝，payload/outer digest 不得自引用或泄露 credential。整组 isolation/Acceptance E2E 必须使用 SD1-02 production adapter 实际签发的 Developer、Reviewer、Orchestrator、Verifier、run-owner bindings，并现场 revoke/rotate 至少一组 identity 后证明旧 binding 被拒；production composition 载入 fixture/mock/static ActorRef 或 test key 必失败。逐个构造 Reviewer 与 Developer、run owner、HumanApprover、Orchestrator、Verifier、Promotion Worker、Journal Anchor 复用 actorId、principalBindingDigest、credentialBindingDigest 或 key，全部 isolation fail；**新 actorId + 新 session + 新 binding digest，但相同 `principalSubjectDigest`/`opaqueCredentialHandleDigest`/key 也必须拒绝**。High-risk reviewer 彼此在 actor/principal/credential/key/session/context 全维唯一；sealed evidence 可相同但 context instance 必须独立。Review-before-freeze、两次 review 之间 ParticipantIdentitySet 变化、集合缺 participant/追加/替换、isolation attestation/report 的 set digest 或 TaskClassAttestation digest mismatch/replay，全部拒绝并作废 Acceptance。覆盖 classification fallback 与 path/task/capability/diff/severity risk trigger、阈值边界和最大 reviewer count；另证明 Reviewer 看不到 Developer transcript/Memory/Plugin/holdout secret，prompt/delimiter/schema/oversize/parser injection→inconclusive，deterministic fail 无法被 advisory 改 pass。Shadow E2E 覆盖 pass/repair/reject/cancel/crash/stale，evidence 绑定 production registry/issuer artifacts 与 exact SHA。
- **Human gate**：由未参与 Developer 实现的 acceptance reviewer 核验 SD1-02 production adapter、真实 issuer/binding/revocation 与 composition evidence，并检查 isolation evidence 和一组 sealed advisory reports；fixture/mock evidence 不得沿用。Product owner 明确接受“模型只建议、人类最终逐项判断”的 shadow UX。

## 8. SD4 — 人工批准与 Branch-only Promotion

### SD4-01 · Human challenge 与 ApprovalReceipt

- **Scope**：在 SD1-02 production identity/trust adapter 上扩展已落地的无自引用 `SelfDevKeyRegistryPayload` + 外部 Binding/ArtifactRef/append-only epoch；key record 按 principal-binding issuer、credential-binding issuer、builtin-classifier-registry issuer、task-class-attestation issuer、human-proof actor、challenge issuer、receipt issuer、journal-anchor issuer 分 discriminator，绑定 usage/domain/issuer/subject/actor-or-service/build/channel/root-purpose/validity/revocation。为此前尚未加入生产参与集的 HumanApprover 配置独立身份与 HumanProof key；Human key 只能签 HumanProof，classifier/challenge/receipt control-plane issuer 使用各自独立 service key；cross-domain/role/key 复用拒绝。提供本地 TTY + recent OS re-auth + OS-protected human key，或独立受信审批界面。按 §18 生成 ApprovalContext，绑定 TaskClassificationResult、signed TaskClassAttestation 与完整 ParticipantIdentitySet；Challenge issuer、HumanProof actor、Receipt issuer 三签逐层绑定 context/challenge/proof 与 registry binding。UI 展示完整 context、classification/participant set、硬门/advisory 和 expected commit/ref transaction。无自动确认。HumanApprover 一旦选定并加入参与集，SD3-03 旧 ParticipantIdentitySet、IsolationAttestation、AcceptanceReport 与 ApprovalContext 全部失效；必须在当前 exact SHA 上重建/冻结包含真实 HumanApprover 的新 set，使用 production adapter **完整重跑 SD3-03 isolation + Acceptance E2E**，由独立 acceptance reviewer 对新 set/evidence 重新签字，才允许进入 SD4-02。
- **Dependencies**：SD3-03；并显式依赖 SD1-02 production identity/trust adapter 门禁，不得用 fixture/mock identity 补 HumanApprover。
- **Tests/evidence**：RegistryPayload canonical bytes 不含自身 artifact/digest，Binding 复算可构造；payload/binding/epoch/previous binding 任一篡改或 rollback 拒绝。对 principal/credential issuer cross-domain、自签 ActorRef、subject/actor/role/purpose mismatch、同 credential 冒充新 actor、human key 冒充 challenge/receipt/journal issuer、service/build/root-purpose mismatch、wrong algorithm、expired/revoked key 做全矩阵 deny。另覆盖仅 TTY 无 OS auth、uid/name/model/service 伪装、pipe/stdin/`--yes`、重放/race、freeze 后 task attestation/participant set/identity binding 或任一 context 字段变化、base/ref race 与 request-changes；nonce/receipt 各自 single-use。新增 post-approver revalidation：旧 ParticipantIdentitySet/report/attestation/context 必拒绝；以 production adapter 签发并验证 HumanApprover identity，重新执行 SD3-03 的全部 role/principal/credential/key/session/context isolation、review-before-freeze、risk/reviewer-count、strict-channel/injection 和 Acceptance shadow E2E；证据必须绑定新 exact SHA、新 ParticipantIdentitySet 和新 Acceptance digests。Fixture/mock/static identity、pre-approver evidence 或只重跑子集均不得计入。
- **Human gate**：BDFL/product owner 做真实交互与最终语义判断验收；security reviewer 审批 trust root、registry/revocation、signature/domain、context freeze、nonce/TTL 和 replay 模型；此前未参与 Developer 实现的 independent acceptance reviewer 必须在包含真实 HumanApprover 的新 exact SHA/ParticipantIdentitySet 上重新审核并签署全部 SD3-03 isolation/Acceptance evidence。三方门禁全部关闭前不得开始 SD4-02，旧签字与 fixture/mock evidence 不得沿用。
...
- **Dependencies**：SD4-01 的 product/security/independent-acceptance 三方门禁及 post-approver 全量重跑、重签 evidence 已全部关闭。
...
- SD1-02 production identity/trust foundation 已在 production composition 上完成 model、service、human-run-owner issuer key provisioning 与 Principal/Credential binding issuance→verification→rotation/revocation E2E；fixture/mock/static/self-signed adapter 或 evidence 未参与任何 readiness 判定。
- SD4-01 加入真实 HumanApprover 后，pre-approver ParticipantIdentitySet/IsolationAttestation/AcceptanceReport/ApprovalContext 已作废；SD3-03 全部 isolation/Acceptance E2E 已在最终 exact SHA、新 ParticipantIdentitySet 和 production identity adapter 上重跑，并由 independent acceptance reviewer 重新签字，SD4-02/03 未沿用旧 evidence。
- SelfDev 默认 off、branch-only；无 push/merge/tag/publish path，无 dangerous bypass。
- TaskClassificationResult、signed TaskClassAttestation、ParticipantIdentitySet、Baseline、VerificationBundle、AcceptanceReport、ApprovalReceipt 与 promotion receipt 绑定同一 run/base/candidate/effective-policy chain；deceptive/unknown/mismatched classification、identity reuse、stale/timeout/inconclusive 全 fail closed。
