...
# Volund CLI · 插件 /status 面板扩展白皮书 (r1)

> **状态**：r1.1（2026-08-25）——**K0 渲染器 + dev 装载路径已落地**：约定目录 `~/.volund/plugins-dev/<name>/` 自动发现（REPL 启动与 `volund status` 均加载；`VOLUND_DEV_PLUGINS` 仅作仓库内开发的额外路径），经 `volund-sandbox --run-plugin` 沙箱子进程激活（不经 Catalog，与冻结中的 legacy 安装路径 `~/.volund/plugins` 完全隔离）；插件数据目录 `~/.volund/plugins-dev-data/<name>/`。Catalog v2 生产路径仍待 CAT-01/02 + ABI-R1。
> **文档类型**：ADR + 扩展规约
...
| Bridge 命名空间 | `volund.ui.status`（§6.4.1a 草拟位置不变） |
| Manifest 权限 | `permissions.volund` 包含 `'ui.status'`（与 tools.register 等同一命名空间，deny-by-default）；桥方法→权限映射见 plugin-runtime `BRIDGE_PERMISSIONS` |
...
**r1.1 已知缺口**：沙箱桥的 `volund` 是泛化 RPC 代理，属性访问不触发 RPC——`volund.plugin.name/version/dataDir` 在沙箱路径下**不可用**（v2 候选：run_plugin 经 argv 注入插件身份）。插件需要用到的自有元数据请自行硬编码或持久化到 dataDir。

### S3.2 描述符 schema（schemaVersion: 1）

```ts
// VolundBridge.ui.status
status: {
...
3. **已落地（r1.1，2026-08-25）**：
   1. `plugin-sdk`：`PluginStatusTabBody` / `PluginStatusTabSpec` / `PluginStatusSectionSpec` + `VolundBridge.ui.status`。
   2. `plugin-runtime`：`PluginBridgeServer`（fd3 JSONRPC 宿主侧 + `callback.invoke` 反向调用）、`activateLocalPlugin`（manifest 校验 → bundle 校验 → 沙箱 profile → `volund-sandbox --run-plugin`）、`createLocalPluginDispatch`（权限 guard + 方法路由）、`BRIDGE_PERMISSIONS` 增 `ui.status.*` / `session.getUsage` / `log.*` 映射；`sandboxProfile` 放行 darwin 系统 OpenSSL 配置（否则沙箱内 node 起不来）。
   3. `packages/ui`：StatusPanel 页签集合动态化 + `sanitizePluginTabs`（§S3.3）+ 三种体例渲染器（heatmap 复用抽取的公共 `StatusHeatmap`）。
   4. `apps/cli`：`localPlugins` 端口（`activateLocal`/`loadBuiltinPlugins`/`loadDevPlugins`/`deactivateAll`；内置插件与 dev 插件同链路，仅发现源不同）；`VOLUND_DEV_PLUGINS=<dir>` 在 REPL 启动与 `volund status` 一次性命令里激活；runtimeStatusData 在面板数据组装时经桥回调 render 取值。
...
4. **解禁后实现清单**（按依赖序）：
   ...
   2. `plugin-runtime`：`volund.ui.status` bridge + permission 检查 + sanitize（guard/上限/凭据拒绝）。
   3. `packages/ui`：StatusPanel 页签集合动态化 + `heatmap`/`table` 渲染器（heatmap 复用 StatsView 抽取的公共组件）。
   4. `apps/cli`：注册表接线 + `volund status --json` 输出插件 section（字段省略规则同 §11.3.14 诚实显示）。
