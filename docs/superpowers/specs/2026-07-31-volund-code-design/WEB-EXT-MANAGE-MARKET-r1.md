...
> **状态**：r1.3（2026-09-20）——**默认市场源落地（D-3 被用户决策推翻）**：Skills 默认源 = Anthropic 官方 skills 仓库市场清单（`.claude-plugin/marketplace.json`，业界 Claude marketplace 格式；走 jsDelivr CDN，raw.githubusercontent 部分网络不可达）；MCP 默认源 = 官方 MCP Registry v0 API（registry.modelcontextprotocol.io，remotes→http / npm→`npx -y` / pypi→`uvx`，仅收 active、按名去重）。marketList 返回增 `isDefault`（UI「默认源」徽标），市场段加条目过滤框；`skill-install` 新增 GitHub tree URL 子目录安装（市场单 skill 安装 + CLI 通用）；fetch 加 15s 超时。r1.2：MG-01~16 已落地，MG-17 裁剪（D-4）；真机冒烟顺带修三个存量缺陷（skill install 惰性 provider 静默装 0 个 / telemetry events 未接线 / 市场 capability 键名复数），登记 `memory_transfer_unavailable` 错误码。r1.1 自评审修订 + r1 首版见 git 历史。
> **文档类型**：ADR + 扩展规约
