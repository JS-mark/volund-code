...
**迁移策略**：
- `volund resume <id>` 读 session 时先读首行 `v`：
  - `v` == 当前 → 正常 replay。
  - `v` < 当前 → 尝试 migration（`storage.migrations[v→v+1]` 链式）；migration 成功 → replay；migration 缺失 → 报错"session 来自不支持的旧版本，建议 `volund history export <id>` 导出后新建"。
  - `v` > 当前（用户降级 volund）→ **明确拒绝** + 报错"session 来自更新版本，请升级 volund 或新建 session"，不尝试向前迁移（避免数据损坏）。
- `volund history import <file>` 支持把老 `v` 的 session 升级迁移到当前版本（best-effort，未知字段保留但不解读）。
...
`volund resume <id>` 默认走 `tailTurns: 20`：只恢复最近 20 个 turn 的 SessionState，更早的消息**不进 messages 数组**，而是：
...
- `volund doctor` 检查 session 文件大小：> 50MB 标 warning + 建议 `/compact`（触发 ContextPolicy 压缩，[§8b](./08b-context-policy.md)）或 `/save <name>` 归档。
- `volund history clear --older-than <date>` 清理老 session（含其 `.index.json`）。
...
**模型解析顺序**（Router 实际使用的 model）：`options.model`（调用方显式覆盖）→ `preferences.model`（状态面板选择，id 形如 `anthropic/<model>`，入 Router 前剥前缀）→ `provider.<name>.model`（静态配置）→ 内置默认。`provider.<name>.baseUrl` 同段生效，决定请求与 `volund login` verify 的打向（网关场景两者通常一起配）。

**★ r13-I4：未知 key 策略与全量 schema**：

- **未知 key → warn + 忽略**（顶层未知 section 与已知 section 内未知 key 均如此；向前兼容——新版本 volund 的 config 在旧版本上不炸）。警告带 key 全名与所在文件，防"打错段名静默失效"（如把 `[context]` 写成 `[contex]`）。
...
> 解决 REVIEW-r7 NEW-P0-1：`cd malicious-repo && volund` 时，仓库自带的 `<cwd>/.volund/config.toml` / `mcp.toml` 自动加载无信任门，可重定向 provider endpoint 偷 API key 或切 telemetry 到 otel 外泄 prompt。

**威胁**：克隆一个恶意仓库后跑 `volund`，仓库内的项目级 config 可含：
```toml
...
1. **首次进入项目信任门**：`apps/cli` 启动时若检测到 `<cwd>/.volund/config.toml`（或 `mcp.toml`）**首次**被加载（无对应信任记录），**必须**弹信任确认（类似 VSCode workspace trust / claude-code 项目设置）：
   ...
   - 信任记录写 `<cwd>/.volund/config.trust.toml`（`{ trusted_at, volund_version, config_hash }`）；config 文件 hash 变化 → 重新弹窗。
...
3. **MCP server 自动连接也走信任门**：`<cwd>/.volund/mcp.toml` 里的 server **首次**连接前同样弹信任（展示 server 名 + transport + 暴露的 tool 数）；与 §11.3.9 `mcp add` 的凭据迁移规则正交。
...
- Layer 4：**用户级 config** `~/.volund/config.toml` 的 `[auth] <provider>_api_key`（显式 opt-in 明文 key，如 `anthropic_api_key`；项目级 forbidden，§8.3.1）—— 最后 fallback

`auth.getCredential(provider)` 按顺序尝试，第一个命中即返回。

**`[auth] skipAuth = true`**（仅用户级 config，项目级 forbidden，§8.3.1）：**完全跳过**凭据解析——provider 请求**不带**凭据头发出（企业网关 / 本地代理等带外认证场景，通常配合 `provider.<name>.baseUrl`）。设了即不再触碰任何 credential 层（含 Layer 4，也不触发 enc 文件 passphrase 提示）；此时交互式 `volund login` 是 no-op（提示现状并退出），仅显式 `--api-key-stdin` 可落盘凭据（附"skipAuth 期间不生效"提示）。与同段 `<provider>_api_key` 并存时 **skipAuth 优先、key 不生效**（`volund doctor` 的 auth 检查会指出该冲突），二者应二选一。进程首次跳过发 `auth.credential.skipped`。

#### 8.4.0a ★ Layer 2 加密文件的安全加固（REVIEW-r6 P1-5）

> 解决离线爆破 `~/.volund/credentials.enc` 无防护的问题。

**KDF（强制）**：passphrase → key **必须**用 **Argon2id**（`m=64MB, t=3, p=2`，salt 随文件存），**禁止**用 PBKDF2/SHA 直接派生（太易爆破）。`argon2` npm 包或 Rust `volund-fs` 导出均可。

**在线 brute-force 防护**（`auth.encfile.unlock` 路径）：
- 连续错误次数累计（持久化到 `~/.volund/auth.state.json`，防进程重启清零）：
  - 1–2 次错误：正常返回失败
  - 第 3 次起：每次错误后**指数退避 sleep**（`min(2^(n-2), 60)` 秒，n=累计错误次数）
  - 累计达 **20 次**：**锁定 24 小时**（`locked_until` 字段），期间任何尝试立即拒绝 + `auth.encfile.locked` telemetry，不接受 passphrase 直到锁定到期或用户用 OS keychain 主密码 / 命令行 `volund auth unlock --reset`（重置需重新加密所有凭据）。
- 每次错误/锁定都发 §8.4.1 的 `auth.encfile.unlock_result` / 新增 `auth.encfile.locked` 事件。
- **强制启用 keychain 提示**：连续 fallback 到加密文件超过 N 次 → UI 红条提示"建议配置 OS keychain（更安全），运行 `volund auth migrate-to-keychain`"。
...
> `volund login --oauth`（§11.3.2）支持 OAuth，但 access token 短期 + refresh token 长期，spec 此前未定义轮换/撤销。
...
- refresh 失败（refresh token 也过期 / 被撤销 / 网络）→ **revoke 本地 refresh token**（清 keychain/enc 文件里的 OAuth 凭据）+ emit `auth.refresh.failed` + 提示用户重新 `volund login`。**不**静默保留失效 token 反复重试。
...
**心智**：auth 是安全 + 用户体验的双热点，必须为**后期数据分析**（不是自动上报）留下完整轨迹。所有事件写入本地 `~/.volund/telemetry/*.jsonl`，遵循 [AGENT.md §4.13](../../../AGENT.md#413-遥测隐私强约束)（默认不出网，仅 OTel opt-in 时上报）。**每一条事件都必须脱敏**（不含 key/token/passphrase 明文）。
...
- `Write` / `Edit` / `MultiEdit` 执行前，如果目标文件存在 → 备份到 `~/.volund/backups/<session-id>/<sha>/<original-path>`（★ 按 `<session-id>` 隔离目录，REVIEW-r6 P1-4：防两个 volund 实例同 cwd 时 backup 互覆）
- `volund restore <session-id>` 可回滚该 session 内的所有变更
- Backups 有 GC 策略：默认保留 7 天 + 500MB 上限，超出按 LRU 清理；GC 由 `~/.volund/state.lock`（flock）保护，同一时刻只有一个 volund 实例跑 GC，防竞态删别人刚写的 backup

#### 8.6.1 ★ 文件级并发写保护（REVIEW-r6 P1-4）

> 解决两个 volund 实例同 cwd → 两个 Runner 同时 Edit `foo.ts` → backup 覆盖 / diff 冲突。

**契约**：`Write` / `Edit` / `MultiEdit` 在**写目标文件前**必须对该文件路径取 advisory file lock（`flock(LOCK_EX)` on Unix / `LockFileEx` on Windows），锁文件 = `<target>.volundlock`（隐藏，写完即删；进程崩溃残留由启动 GC 清理 `*.volundlock` 超过 1 小时未变的）。

- 取锁失败（另一实例持锁）→ **等待 + 重试 3 次**（每次 1s），仍失败 → 拒绝写 + 返回 `tool_result.isError` "file locked by another volund session (<pid>), retry later"，**不**强行覆写。
...
- 单步语义：每次 `/undo` 回退一步（恢复该次 Write/Edit/MultiEdit 的 backup）；连续 `/undo` 依次向前。v1 不提供 `/undo N` 跳步与列表选择（`volund restore <session-id>` 是全量回滚的另一入口）。
...
**已在 [AGENT.md §4.13](../../../AGENT.md#413-遥测隐私强约束) 强制**：默认写 `~/.volund/telemetry/*.jsonl`，不出网。
