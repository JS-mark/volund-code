...
> L1 的核心 DoD：**能用 `volund` 对着 Anthropic 完成一个真实的编码任务（改文件 + 跑测试 + 提 PR），全程权限弹窗可控；mac/linux 4 target 的 escape 测试基础用例全部通过，任何降级都在 UI 显式披露**。
>
...
- [ ] 👤 **真实编码任务跑通**（dog-fooding，闸门第 7 项）：用 `volund` 完成 ≥1 个真实编码任务，覆盖「读文件 → 改文件 → 跑测试」全链路
...
- [ ] ⚙️ **native job 全绿（4 target）**：
  ...
  - [ ] Linux target: `volund-sandbox --verify-bwrap-digest` 校验 bundled bwrap SHA256 通过
...
- [ ] ⚙️ **`volund doctor --strict` 全绿**（mac/linux 4 target 各跑一次，[§11.3.10](./11-cli-commands.md)）：node version / Volund version / native-bridge available / auth / config valid / cwd writable
...
- [ ] 🛡️ **sandbox profile 禁止放宽 PermissionSpec 声明的权限**（crates/volund-sandbox 单元测试）
- [ ] 🛡️ **sbpl 路径转义 / seccomp 架构分表 强制单元测试覆盖**
- [ ] ⚙️ **Rust crate 禁止发布 npm**（pnpm workspace 配置）
- [ ] ⚙️ **`volund-search` 结果尊重 `.gitignore` / `.volundignore`**（单元测试）
...
- [ ] ⚙️ **sandbox binary 是独立进程，不 dlopen 进 Node**（crates/volund-sandbox 是 bin 不是 lib）
...
- [ ] 🛡️ **@include 仅在 volund 提示词管线展开**（Read/Grep/Edit 不展开，PromptLoader 与 Read 是不同代码路径，单元测试）
- [ ] 🛡️ **展开路径落在 workspace 或 ~/.volund 双白名单内**（canonicalize 检查，单元测试）
...
- [ ] ⚙️ **rolldown 打 apps/cli 单 bin**（`dist/volund.js` + shebang，package.json `"bin": {"volund": "dist/volund.js"}`）
- [ ] ⚙️ **changesets 发版**（L1: 12 平台包 = 4 target × 3 crate + volund-code + fs-common）
...
- [ ] 👤 **闸门 7**：至少一次真实使用（dog-fooding，用 volund 开发 volund）
...
**签收人**：__________ **日期**：__________ **volund 版本**：v0.1.0
