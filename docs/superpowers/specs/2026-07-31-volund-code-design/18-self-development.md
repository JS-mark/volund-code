...
>
> **SD4 effect/ABI override：** Human prompt前冻结同一`SelfDevPromotionPlan`的Catalog/Git effect plans；§18的K3 `ApprovalReceipt`/`ApprovalConsumption`唯一映射为§19a `PromotionApprovalReceiptV1`/`promotion-approval-consumption.v1`，不存在并行旧approval authority。Approval、APPROVED→PROMOTING consumption与PROMOTING→COMPLETED/FAILED都采用§19a无环 anchored protocol。Primary `PromotionCoordinationStore`先写durable PREPARED锁；独立monotonic SelfDev AnchorStore以OPEN→ANCHORED|CANCELLED CAS作为logical transition commit，签event/checkpoint/anchor与durable K0 final artifact，随后primary幂等FINALIZED。任何reader遇PREPARED必须先resolve anchor outcome；禁止cross-store eventual absence或声称两个store物理原子。Stage/Git只写各自effect ledger/receipt，不制造PROMOTING→PROMOTING run transition。Completion FINALIZED必须把exact PROMOTING run→COMPLETED、reservation `STAGE_PENDING→RELEASED_COMPLETED`并清pointer；`STAGED_DISABLED`唯一投影是同一finalized snapshot中的immutable `RELEASED_COMPLETED` history + matching anchored SelfDevCompletionReceipt，任何仍处pending的reservation/receipt组合都不能投影。普通expiry仅在non-AMBIGUOUS、linearizable全ledger absence proof下anchored转run `FAILED/recovery_failed`、reservation `SUPERSEDED_EXPIRED`并清pointer。AMBIGUOUS不得TTL supersede：promotion old run必须一次转terminal `FAILED/recovery_failed`且永不恢复；ordinary invocation只关闭/revoke其context。只有purpose-signed per-effect external reconciliation、全部sibling terminal proof与aggregate lineage release完成后才能释放coordination pointer，且FAILED run head/version/anchor不再改变；继续工作只能由人启动全新run并绑定exact reconciled dependency，不能自动重放。Consumer/stage/git/completion worker的lease/RunWorkerFence与per-capability reservation fence是不同nominal authority，pre-anchor actor与durable finalizer也不得冒充。Adoption/enable仍各需fresh独立人工receipt；本节其余冲突处以§19a ABI-00为normative override，状态机、证据、独立验收和人工确认要求继续有效。
>
...
建议 CLI namespace 为 `volund selfdev`，与现有 `volund evolution show|rollback`（§15 参数调优）严格分离：

```text
volund selfdev propose --goal-file <path> --base-ref <full-ref> --base-sha <sha> [--task-class-hint <id>] [--promotion-ref <new-local-ref>]
volund selfdev start <runId>
volund selfdev status <runId> [--json]
volund selfdev evidence <runId>
volund selfdev cancel <runId> --reason <text>
volund selfdev request-changes <runId> --reason <text>
volund selfdev reject <runId> --reason <text>
volund selfdev approve <runId>
```
...
| Public API / `volund selfdev` CLI | **Proposed / not shipped** | 命令 namespace 仅为提案 |
