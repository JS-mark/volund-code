...
| Web 扩展管理 + 市场白皮书 (WEB-EXT-MANAGE-MARKET r1) | [`WEB-EXT-MANAGE-MARKET-r1.md`](./WEB-EXT-MANAGE-MARKET-r1.md) | Web 四域（MCP/Skills/Memory/Plugins）动作补齐（复用既有 port/domain，零新写路径）+ memory markdown 编辑器 + McpManager.reload 重读配置修复 + 管理 Page「市场」页签（Plugins 复用 `[plugins] market` / Skills git 目录索引 / MCP 预填表单目录）；任务卡 MG-01~17（proposed） |
