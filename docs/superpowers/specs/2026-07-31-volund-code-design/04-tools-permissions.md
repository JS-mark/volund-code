...
| `ShellOutput` | 后台任务 | 查看等待后台 shell 输出（r13-G2，L1 随 Bash） | ✅（只读缓冲） | 继承对应 shell 的 profile | volund-sandbox |
| `KillShell` | 后台任务 | 终止后台 shell（r13-G2，L1 随 Bash） | ❌ | 继承对应 shell 的 profile | volund-sandbox |
...
> ★ **自我进化接入（r10）**：各 tool 的 `timeoutMs` 可经 [§15.4](./15-self-evolution.md) 自调优；调整记 `~/.volund/tuning/tool-timeout.jsonl`。观察信号：tool 超时频率 / 超时后用户重试频率。落地里程碑：L3。安全护栏：timeout 上限不可超过 300s（防进化让恶意 tool 长期占用）。
...
- **并发保护双闸**：
  1. 写入前 lockfile（`<path>.volundlock`；重试 3 × 1s；仍占用 → `isError` 报错含持锁 pid）；
...
- **L3**：`Task` + subagent；`Memory.recall`（走 volund-search 索引）；MCP 工具注入；插件工具注入
