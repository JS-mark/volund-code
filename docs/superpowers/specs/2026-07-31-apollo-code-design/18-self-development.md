...
| **Independent Reviewer** | sealed diff、目标、VerificationBundle、允许的只读证据 | 签发 `AcceptanceReport` | 读开发会话/草稿/私有 Memory、调用写工具、修改 evidence、批准 promotion |
| **Human Approver** | 完整绑定摘要、diff、验证与验收报告、预算与安全结果 | 签发 `ApprovalReceipt`，或 request-changes/reject | 用通用 `--yes`、脚本 stdin 或模型代理替代本人确认 |
...
| `BASELINING` | `STALE` | Orchestrator | `baseRef` head 已不等于 `baseSha` |
...
| `ACCEPTING` | `AWAITING_HUMAN` | Independent Reviewer 申请、Orchestrator 执行 | isolation attestation 有效；recommendation=`accept`；AcceptanceReport 完整绑定 verification 和 PromotionPlan |
| `ACCEPTING` | `REPAIRING` | Orchestrator | recommendation=`repair`；finding 可修复且 repair budget 尚有余额 |
| `ACCEPTING` | `REJECTED` | Independent Reviewer 申请、Orchestrator 执行 | recommendation=`reject`；report digest 已持久化 |
...
| `AWAITING_HUMAN` | `STALE` | Orchestrator | `baseRef` head 已变化；所有 challenge 作废 |
...
| `APPROVED` | `STALE` | Orchestrator | receipt 已过期或 `baseRef` head 已变化；receipt 作废 |
...
6. **幂等副作用**：每个 effect 由稳定 `effectKey` 唯一标识；恢复时先查询 receipt，再决定重试。禁止因崩溃重复创建 worktree、重复执行 promotion 或重复消费批准。
7. **lease 有界**：Developer、Verifier、Reviewer、Promotion Worker 都使用短租约；租约到期只允许安全停止和恢复，不允许默认继续。
8. **审批不可转移**：Developer、Reviewer、Orchestrator 和 Promotion Worker 不能成为该 run 的 Human Approver；同一模型的不同 prompt 不构成人类审批。
9. **审计不可删除**：取消、失败、拒绝和清理不能删除 journal、manifest、evidence digest 或 receipt 使用记录。

### 18.5 策略优先级、受保护面与 Runner profiles

有效能力必须按下式计算，并在 `PROPOSED → BASELINING` 时固定 digest：

```text
effectivePolicy = builtinMaximum ∩ userPolicy ∩ policyLoadedFrom(baseSha)
```

三层都只能**收窄**权限。候选内的 policy、测试脚本、AGENT/CLAUDE 指令、Skill、Plugin、Hook、package script 或 CI 变更，不影响当前 run。策略读取失败、版本未知或交集无法证明时，run 失败关闭。
...
- Human approval store、actor/nonce/TTL 校验、tamper-evident journal、artifact store 和 promotion worker。
...
- baseline 与 candidate 使用同一 suite、镜像、依赖缓存 digest、环境变量、资源限制和时钟策略；无法证明同环境则结果为 inconclusive。
- candidate 修改的 package scripts、测试配置或 snapshots 不得改变 required suite 的命令图；如 suite 必须加载候选配置，policy 必须显式把该配置视为被测输入而非可信控制面。

### 18.6 权威数据模型

以下是 v1 最小持久模型。所有 `*Digest` 使用版本化 canonical encoding + SHA-256；schema 版本和 canonicalizer 版本必须进入 digest 域。
...
interface Signature {
  algorithm: 'Ed25519'
  keyId: string
  domain: 'selfdev.approval-challenge.v1' | 'selfdev.approval-receipt.v1'
  valueBase64: string
}
...
type ReasonCode =
  ...
  | 'recovery_failed' | 'human_rejected' | 'human_cancelled' | 'stale_base'
...
interface SelfDevRun {
  ...
  environmentDigest?: Digest
  lease?: { role: SelfDevRole; holder: string; tokenDigest: Digest; expiresAt: Timestamp }
  budget: RunBudget
...
interface PromotionPlan {
  ...
  gitTreeObjectId: GitObjectId
  commitAuthorPolicyDigest: Digest
  commitMessageDigest: Digest
  commitTimestamp: Timestamp           // fixed before approval; no promotion-time entropy
  planDigest: Digest
}
...
interface ApprovalReceipt {
  ...
  expiresAt: Timestamp
  decision: 'approve'
  signature: Signature & { domain: 'selfdev.approval-receipt.v1' }
...
interface HumanProof {
  actor: ActorRef
  challengeDigest: Digest
  exactResponse: string                // ephemeral; journal stores only its digest
  signature: Signature & { domain: 'selfdev.approval-challenge.v1' }
}
```

`ArtifactRef.storageKey` 只是定位提示，消费前必须按 `sha256` 重读校验。Journal 以 `previousEntryDigest` 形成 hash chain；周期 checkpoint 必须绑定链头、run version 和 store generation。截断、重排、重复 sequence 或未知 schema 都导致 fail closed。

所有 digest 使用 domain-separated canonical encoding，例如 `selfdev.approval-receipt.v1 || canonicalPayload`，不能只 hash 字段拼接。Challenge nonce 在 `AWAITING_HUMAN → APPROVED` 的 CAS 中消费一次；`ApprovalReceipt` 是另一种 single-use capability，在 `APPROVED → PROMOTING` 的 CAS 中消费并生成 `ApprovalConsumption`；`PromotionReceipt` 只证明 Git effect 的幂等结果，三者的用途和消费记录不得复用。
...
interface SelfDevelopmentService {
  ...
  approvalChallenge(runId: string, actor: ActorRef): Promise<ApprovalChallenge>
  approve(challenge: ApprovalChallenge, proof: HumanProof): Promise<ApprovalReceipt>
}
```
...
1. 必须是经过认证的授权 actor 和交互式 TTY；先以 CAS 重读 `AWAITING_HUMAN` 的最新版本。
2. UI 完整显示 `runId`、`baseRef@baseSha`、新建的 `promotionRef`、`candidateDigest`、`policyDigest`、`promotionPlanDigest`、`verificationBundleDigest`、`acceptanceReportDigest`、commit parent/tree/message/author plan、diff 摘要、required/holdout/safety 结果、预算消耗、repair 次数和 receipt TTL。
3. 控制面生成至少 128-bit 随机一次性 nonce；用户必须准确输入 UI 给出的 `approve <candidate-short-digest> <nonce-challenge>`。短 digest 仅用于人机挑战，receipt 仍绑定完整 digest。
4. 服务端重新计算全部完整 digest，校验 actor、TTL、nonce 未使用、`baseRef` head 仍为 `baseSha`、`promotionRef` 仍不存在，随后在同一 CAS 中消费 nonce 并签发 `ApprovalReceipt`。默认 TTL 由 builtin policy 限定，用户 policy 只能缩短。
5. 不提供 `--yes`、`--force`、环境变量确认、pipe/stdin 自动确认或模型 tool-call 确认。非 TTY 只能导入由独立受信审批界面/硬件身份签发的同结构签名 receipt，不能降级成布尔值。
6. request-changes、reject、过期、base 变化或任一 digest 变化都使旧 challenge/receipt 永久失效。

Human approval 只授权**该 ApprovalReceipt 绑定的 PromotionPlan**：精确 parent=`baseSha`、tree=`gitTreeObjectId`、commit metadata plan 和新本地 `promotionRef`。进入 `PROMOTING` 时 receipt 再作为 single-use capability 原子消费；它不授权 push、merge、tag、publish、release、其他 ref 或未来修复 generation。

### 18.8 预算、修复、崩溃恢复、取消与回滚

`RunBudget` 至少包含 wall-clock、模型 token/费用、tool calls、变更文件数、增加/删除行、单文件大小、scratch/artifact 字节、suite 次数、并发数和 repair attempts。effective policy 对每项给出硬上限；缺失上限不是无限，而是不能启动。超限先原子记录原因，再停止 Runner，不能靠删除 evidence 续跑。
...
- Orchestrator 从 journal + checkpoint 纯函数重建状态；发现链断裂或 store generation 回退即停止。
- 外部 effect 执行前写 intent，执行后写 receipt；恢复时用 `effectKey` 查询真实状态，不能盲重放。
- 在状态跃迁、文件 seal、suite 完成、approval 消费和 ref 创建之间的每个崩溃点都必须有故障注入测试。
- 过期 lease 由恢复 worker 撤销；只有 CAS 成功的新 lease holder 可继续。
...
- promotion 只按已批准 PromotionPlan 原子创建一个新的本地 `promotionRef`（缺省可确定性派生为 `refs/heads/selfdev/<runId>`）；其 parent 必须为 `baseSha`，tree/metadata 必须与 plan 一致。`COMPLETED` 后系统不自动删除或重写该 ref；人类可用普通 Git 流程检查、改名、合并或删除。
- promotion 前 `promotionRef` 必须不存在；已存在但不能由同一 effect intent/精确 commit 对账时是 `promotion_conflict`。若 ref 已由本 effect 正确创建但 PromotionReceipt 未记账，恢复应补记并转 `COMPLETED`，而非创建第二个 ref 或接受不同 commit。
...
4. **Isolated model reviewer**：只在前三层明确完成后评估语义正确性、设计一致性、风险和可维护性；模型报告不能把 deterministic fail 改成 pass。
...
SD4/v1 的 promotion 只完成三件事：验证 `baseRef@baseSha` 与 ApprovalReceipt/PromotionPlan 绑定、以 plan 中精确 parent/tree/metadata 生成或验证本地 commit、原子创建 plan 指定且事前不存在的 `promotionRef`。它不修改 current branch/index/worktree，不执行 hooks，不读取用户 Git credential，不访问网络。
...
| 威胁 | 必须的阻断 |
|---|---|
| 候选文档/代码 prompt injection | 全部候选内容标为 untrusted；Orchestrator 不解释；Reviewer 无工具且隔离 |
...
| TOCTOU / seal 后写入 | 撤销 lease、只读 mount、manifest/tree digest、验证前后复算 |
| 自批、自审或共享记忆污染 | 角色身份约束、Reviewer 新会话、无 persistent Memory/Skill/Plugin、HumanProof |
| crash 重放双重副作用 | CAS、lease、intent/receipt、稳定 effectKey、故障注入 |
| base 已过时仍 promotion | approval 前和 promotion 前两次 freshness check；不自动 rebase |
| approval replay/替换候选 | 全 digest 绑定、nonce single-use、TTL、actor signature、journal consumption |
...
| 阶段 | 能力边界 | 可量化退出标准 |
|---|---|---|
| **SD0 — Contract & security prerequisites** | 冻结本节、threat model、路径分类和 release scope；先关闭会影响专用 Runner 的已知权限/Hook P0 | 文档/链接/配置/事件检查全绿；SAFE_BASH 前缀注入 corpus 全拒绝；builtin Hook 对超限 payload 全量 fail-closed 且原始/扫描 digest 一致；安全评审签字。无产品入口 |
| **SD1 — State, store & sealing** | reducer、CAS/lease、hash-chain journal、ArtifactRef、worktree、manifest/seal；使用 fake worker | 所有允许和禁止跃迁表驱动测试 100% 通过；每个 effect 边界故障注入恢复 exactly-once；篡改/截断/stale-base fixture 100% fail-closed；无模型开发入口 |
...
| **SD3 — Verification & acceptance shadow** | `Check.run`、同环境 baseline/candidate、holdout、deterministic graders、isolated reviewer、完整 evidence | required + holdout fixtures 可重复两次且 digest 相同；环境差异/timeout/missing evidence 全为 inconclusive；Reviewer isolation 测试证明无 Developer transcript/Memory/工具；shadow E2E 覆盖 pass/repair/reject/crash/cancel，promotion API 仍不存在 |
| **SD4 — Human-approved branch-only** | approval challenge/receipt、TTL/nonce/actor、freshness、promotion worker、本地 branch E2E | 无 TTY/`--yes`/stdin/replay/过期/替换 digest 全拒绝；base race 阻断；支持平台 E2E 均不改 current worktree 且无远端请求；独立安全审查和人类 release gate 通过。**这是最早 production-capable 阶段** |
