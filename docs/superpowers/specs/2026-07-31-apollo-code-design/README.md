...
| /plugins 管理面板 + 市场链路白皮书 (PLUGIN-MANAGER r1) | [`PLUGIN-MANAGER-r1.md`](./PLUGIN-MANAGER-r1.md) | 三页签浏览（builtin/dev/market）+ CommandTabsView 描述符 + `apollo.plugins.*` 桥 + `[plugins] market` digest 安装链（`~/.apollo/plugins/`，激活期完整性重验）；签名/吊销留给 Catalog v2 |
