...
2026-08-25 起，**CLI 整体**的 npm 分发改走二进制薄壳：`apollo-code` meta 包只剩一个 CJS 壳（`bin/apollo.cjs`），按宿主 triple 解析 optionalDependencies 里的 `@apollo-code/<triple>` 平台包并 spawn 其中的 bun 单文件二进制；平台包内含完整 standalone 布局（二进制 + native/ + plugins/），是 GitHub Release `apollo-standalone-<triple>.tar.gz` 的同内容 npm 形态。与本 ADR 的边界：
...
- `win32-arm64-msvc` 无 bun 编译目标：npm 上由 `@apollo-code/win32-x64-msvc`（`cpu` 同列 arm64）经 Prism 仿真覆盖；**JS 渠道**的 win-arm64 sidecar 仍按本 ADR 走 Release 资产。
