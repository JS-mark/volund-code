...
# Apollo Code · `/skills` 与 `/mcp` 命令 + 协议对齐白皮书 (r1)

> **状态**：r1.8（2026-08-26）——skill 安装支持嵌套仓库(plugins/<name>/skills/…),逐失败续装——SSE 兼容回退 + MCP 结构化日志——「列表 + 开关 + 详情」：**Enter = 详情**（/skills 为 SKILL.md 只读滚动视图、/mcp 为元数据 + 工具清单）、**Space = 启停开关**（行首 ●/○/✘/◌ 图标）——**SM-01~06 + §S3.3a（invocation 语义）+ §S3.7 CLI 管理命令族已落地**（skills 多作用域/标准字段/`/skills` 面板、mcp.toml+`.mcp.json` 装载/McpManager 接线 `mcp__server__tool`/`/mcp` 面板；vitest + pty 实测通过）。**SM-07（OAuth）/SM-08（Streamable HTTP、插件捆绑 skills）未做**。已知实现偏差（vs §S3 原文）：① `/skills` 详情页为只读滚动视图（Enter 打开，j/k 滚动），未做动作菜单与 `$EDITOR`/Copy path；② MCP 无自动重连退避（手动 `r` reload），fatigue 三件套与 keyref 凭据解析未接线（headers 里 `keyref://` 原样透传）；③ §S3.8 telemetry 事件未加；④ ~~`apollo skill/mcp` CLI 子命令族未加~~（r1.4 已补：skill install/uninstall/list/show/enable/disable + mcp add/remove/enable/disable，含 git 安装源与 `--` 透传，见 §S3.7）；r1.7：SSE 规范回退（先 POST initialize 探测，4xx/405 回退 GET SSE）+ `~/.apollo/mcp.log` JSONL 诊断（manager.init/connect.start/connect.ok/connect.failed/connect.needs-auth/disconnect/server.stderr/connect.protocol，追加写、失败静默）；⑤ per-skill 命令的 args 作为整体任务文本附在 skill 框架后（Claude Code 的 $ARGUMENTS 占位插值留 r2）。
> **文档类型**：ADR + 扩展规约
> **范围**：`packages/skills-runtime`（多作用域 + 标准字段）、`packages/mcp-client`（接线 + Streamable HTTP）、`packages/ui`（SkillsPanel / McpPanel）、`apps/cli`（REPL 命令 + `apollo skill`/`mcp` 子命令增补）、`packages/config`（`[skills]` / mcp.toml schema）
...
| `/mcp` REPL 命令 | **不存在**（spec 仅顶层 `apollo mcp` CLI 族；`packages/mcp-client` 已写好但 runtime 未接线） | 新增 `/mcp` 管理面板（SM-06；业界单数惯例，不设复数形式） |
| Skill 文件协议 | 自定义 frontmatter（`apolloVersion` 必填、`resources`、`activation`），单一 `~/.apollo/skills` 作用域 | 对齐 [agentskills.io](https://agentskills.io/specification) 开放标准：标准字段必填、Apollo 扩展降级为可选；新增项目级 + `.agents/skills/` 互操作路径（SM-01） |
...
- **不做** skill marketplace / `skills.sh` 安装源集成（`apollo skill install <spec>` 维持本地目录 + git URL 两种，v2 再议）。
- **不做** MCP server 端实现（Apollo 只做 client）；`resources` / `prompts` / sampling / elicitation 原语的**消费**放 v2——L3 只消费 `tools/list` + `tools/call`（现 `packages/mcp-client` 已覆盖），`/mcp` 面板的 inspect 页展示 resources/prompts 清单但不可调用。
...
### S2.1 vs §6.5.2 现行 frontmatter（`apolloVersion` 必填）

**张力**：开放标准只有 `name` / `description` 必填，无 `apolloVersion` 概念；标准外顶层字段应放 `metadata`。现行实现把 `apolloVersion` 设为必填，导致第三方标准 skill（anthropics/skills 仓库等）无法直接安装。

**消解**：采用**双读迁移**——标准字段（`name`/`description`）必填且约束升级（name 1–64 字符 `^[a-z0-9]([a-z0-9-]*[a-z0-9])?$`、不得连续 `--`、**须与父目录名一致**；description 1–1024 字符）；`apolloVersion` 降级为可选（缺失 = 视为兼容，不再 warning；出现时语义映射到标准 `compatibility` 字段处理）。存量 skill 不需要改动即可继续工作。详见 §S3.1。

### S2.2 vs 单一 `~/.apollo/skills` 作用域
...
**消解**：新增作用域，优先级 **project > user > plugin**（与 Codex/Gemini 的"项目覆盖用户"一致；Claude Code 的"个人>项目"不采纳——Apollo 的项目级输入已有信任门兜底，覆盖语义与 §8 config 分层一致）。同时**只读**识别互操作路径：项目级 `<cwd>/.agents/skills/` 与用户级 `~/.agents/skills/`（Apollo 自有路径 `.apollo/skills/` 优先级高于同层 `.agents/skills/`）。项目级目录（含 `.agents/skills/`）首次发现走 §8.3.1 信任门（展示 skill 名 + 来源路径清单）。
...
**张力**：Apollo 配置体系是 TOML；业界项目级共享配置是 JSON `.mcp.json`。二选一会牺牲一侧。

**消解**：**自有读写走 TOML，互操作只读走 JSON**——`apollo mcp add` 写 `~/.apollo/mcp.toml` / `<cwd>/.apollo/mcp.toml`（顶层 `[mcp_servers.<name>]` 表，字段与业界 JSON 条目同构：stdio `command/args/env/cwd`，远程 `type/url/headers`）；`<cwd>/.mcp.json` 与 `<cwd>/.agents/mcp.json`（opencode 等互操作候选）**只读导入**、条目并入 project scope、走信任门。两条路径同名时 TOML 优先（自有配置可覆盖仓库下发的 JSON）。
...
license: MIT                  # 可选
compatibility: "requires node>=20"   # 可选，≤500 字符；apolloVersion 出现时映射到此语义
metadata:                     # 可选，string→string；扩展属性一律放这里（加前缀防冲突）
  apollo-version: "^1"
  version: "1.0.0"
allowed-tools: "Bash(git:*) Read"    # 可选（实验性）。激活时映射为 permissions 预授权规则

# ── Apollo 扩展（可选；存量字段继续识别，双读）──────────────
apolloVersion: ^1.0.0         # 可选。存量字段；缺省 = 兼容；出现时按 compatibility 语义校验
...
| `apolloVersion` 主版本不匹配 | warning + 面板 `incompatible` 态（现行语义保留，但从必填检查降为存在才检查） |
...
| scope | 路径 | 信任门 | 说明 |
|---|---|---|---|
| `project` | `<cwd>/.apollo/skills/` | 是（§8.3.1） | 随仓库分发；覆盖同名 user skill |
| `project`（互操作） | `<cwd>/.agents/skills/` | 是 | 只读语义（Apollo 不写此目录）；同层低于 `.apollo/skills/` |
| `user` | `~/.apollo/skills/` | 否 | 现行唯一路径，不变 |
| `user`（互操作） | `~/.agents/skills/` | 否 | 只读；同层低于 `~/.apollo/skills/` |
...
- `installFromDirectory` / `apollo skill install` 默认写 user scope；`--scope project` 写 `<cwd>/.apollo/skills/`（交互模式自动记信任）。
...
```
/skills                    # 打开管理面板（本契约核心）
/skills list               # 非面板：transcript 打印纯文本表格（等价 apollo skill list）
...
行首图标：`●` 启用 / `○` 停用 / `✘` broken / `◌` shadowed（broken/shadowed 的 Enter 无效，原因显示在描述列）。SKILL.md 全文查看也可走 `apollo skill show`（r1.4）。
...
项目级 `.apollo/config.toml` 可设同名键覆盖用户级（走既有 config 合并 + 信任门）；`disabled` 名单合并（并集）。

### S3.5 MCP 配置结构

**`~/.apollo/mcp.toml` / `<cwd>/.apollo/mcp.toml`**（顶层表，字段与业界 JSON 条目同构）：
...
**互操作只读导入**：`<cwd>/.mcp.json`（业界格式：顶层 `mcpServers` 对象，stdio `command/args/env`，http `type/url/headers`）→ 条目并入 project scope；`.apollo/mcp.toml` 同名条目优先。`.mcp.json` 走 §8.3.1 信任门 + 启动时首个信任弹窗展示其 server 清单（transport + url/command 摘要）。

**Scope 优先级**：project（`.apollo/mcp.toml` > `.mcp.json`）> user（`~/.apollo/mcp.toml`）。同名整条覆盖，不合并字段（Claude Code 语义）。`apollo mcp add -s user|project`（默认 user）。
...
| `r` | 对全部 server 重跑连通性测试（等价 `apollo mcp test --all`；面板内联显示进行中） |
...
- `McpManager` 把生命周期事件追加写 `~/.apollo/mcp.log`（JSONL）：`manager.init` / `connect.start`（server、scope、transport 摘要）/ `connect.ok`（protocolVersion、tools 数）/ `connect.failed` / `connect.needs-auth` / `disconnect` / `server.stderr`（stdio 子进程 stderr 逐行）/ `connect.protocol`（HTTP 探测结果）。写入失败静默（不阻塞主链路）；行数不设上限（追加式，跨会话累积，清理随用户/磁盘）。
- `~/.apollo/mcp.log` 与 telemetry 事件（§S3.8）正交：日志是排障留痕，telemetry 是采样指标。
...
- 面板动作全部有 CLI 等价物（`apollo mcp list/test/inspect/remove` + 新增 `enable/disable/login/logout/reload`）；面板不产生 config 之外的状态。
- 非交互模式（`--no-tui`/非 TTY）：项目级 `.mcp.json` / mcp.toml 默认不加载（§11.6 既有规则），`enable_all_project_servers` 显式放行。

### S3.7 CLI 命令增补（§11.3.8 / §11.3.9 修订）

```
apollo skill list [--scope user|project] [--json]         # r1.4 已实现：scope 过滤 + status 列
apollo skill enable|disable <name>                        # r1.4 已实现（config [skills] disabled 持久面）
apollo skill show <name>                                  # r1.4 已实现（打印 SKILL.md 全文）
apollo skill install <spec> [-s user|project]             # r1.4 已实现：spec = 本地目录 | git URL |
...
apollo skill uninstall <name> [-s user|project]           # r1.4 已实现（仅可写非 interop 作用域）
apollo skill validate [<dir>]                             # 未做（doctor 侧覆盖，后置）

apollo mcp add <name> -- <command> [args...]              # r1.4 已实现：stdio `--` 透传
apollo mcp add [-t http|sse] <name> <url>                 # r1.4 已实现：远程；-e K=V / -H 'K: v' 可重复；
                                                          #   -s user|project 选目标 mcp.toml（默认 user）
apollo mcp remove <name> [-s user|project]                # r1.4 已实现（默认先查 project 再查 user）
apollo mcp enable|disable <name>                          # r1.4 已实现（config [mcp] disabled 持久面）
apollo mcp list / test / inspect                          # r1.4 已实现（list 连通性有界等待 4s 后快照；
                                                          #   stdio 子进程在 CLI 一次性进程收尾关闭）
apollo mcp reload                                         # 未做（REPL /mcp reload 已有）
apollo mcp login|logout <name>                            # SM-07 未做
```
...
| SM-02 | `[skills]` config 键 + `apollo skill enable/disable/validate/list --scope` + `Skill.activate` 尊重 disabled/disable-model-invocation | `packages/config`、`apps/cli/src/commands/skill/` | L2 |
...
| SM-06 | `/mcp` 面板（含 reload/auth 子命令壳）+ `mcp` 进 builtin 名单 + `apollo mcp enable/disable/reload` | `packages/ui`、`apps/cli` | L3 |
| SM-07 | OAuth 2.1 + PKCE + DCR 客户端流（401 检测 → 浏览器授权 → token 存 auth）+ `apollo mcp login/logout` + 面板 Authenticate | `packages/mcp-client`、`packages/auth` | L3 后段 |
