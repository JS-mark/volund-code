...
Volund has two intentionally different machine-output shapes.

- `volund chat ... --json` and the default prompt entry point use a versioned NDJSON stream. Each stdout line is one complete JSON event. Ink, banners, progress text, and ANSI styling are disabled; diagnostics belong on stderr.
