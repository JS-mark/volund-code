...
Interactive Chat has a separate local-plugin v2 lifecycle through `/plugins`. Market installation only downloads, verifies, and records the plugin; it never activates code. Run `/plugins inspect <name>`, review the complete permissions and permission hash, then `/plugins approve <name> <permission-hash>` and `/plugins enable <name>`. A version or permission-hash change revokes approval and disables the plugin. `/plugins disable <name>` stops it without uninstalling it. This state is stored atomically in `~/.apollo/plugin-state.v2.json`; it never reads or rewrites legacy `~/.apollo/plugins/plugins.json`.
