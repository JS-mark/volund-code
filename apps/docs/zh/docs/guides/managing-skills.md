...
Skill 是提示层能力包：一个目录 + 一份 `SKILL.md`(YAML frontmatter + Markdown 指令)，可选附 `scripts/`、`references/`、`assets/`。Apollo 对齐 [agentskills.io](https://agentskills.io) 开放标准，与 Claude Code / Codex / Cursor 等共用同一文件格式。
...
apollo skill install anthropics/skills          # 同义简写
...
也可以用生态自带工具安装，Apollo 会自动发现（互操作路径，只读）:
...
| 操作 | REPL | CLI |
|---|---|---|
| 列表/状态 | `/skills` 面板 | `apollo skill list [--scope user\|project]` |
| 详情 | 面板里 Enter（只读滚动视图） | `apollo skill show <name>` |
| 启停（持久） | 面板里 Space | `apollo skill enable\|disable <name>` |
| 重扫描 | 面板里 `r` | — |
| 卸载 | — | `apollo skill uninstall <name>` |

启停持久写在 `~/.apollo/config.toml` 的 `[skills] disabled` 名单；重扫后 SKILL.md 编辑免重启生效。
