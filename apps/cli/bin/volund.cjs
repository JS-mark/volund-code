#!/usr/bin/env node
// Volund CLI 的 npm 薄壳：真正的可执行文件是按平台分发的 bun 单文件二进制，
// 装在 optionalDependencies 的 @volund/<triple> 平台包里。本壳只负责
// 解析宿主平台对应的包并把调用转发过去（stdio 直通、退出码/信号透传）。
//
// Windows arm64 说明：bun --compile 不支持 bun-windows-arm64，该平台装
// @volund/win32-x64-msvc（cpu 字段含 arm64），经 Prism 仿真运行；
...
try {
  ...
  fail(
    `platform package @volund/${triple} is not installed (optional dependency skipped?). ` +
      `Reinstall with optional dependencies enabled, or download the standalone archive from https://github.com/JS-mark/volund-code/releases`,
  )
}
