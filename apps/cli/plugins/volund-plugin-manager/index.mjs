 ...
 * 与 dev/builtin 同一条装载链路（volund-sandbox --run-plugin + fd3 桥）。
 */
const shortName = (name) => name.replace(/^volund-plugin-/, '')

function loadedEntry(plugin) {
  const lifecycle = plugin.lifecycle
  const phase = lifecycle?.loaded
    ? 'loaded'
    : lifecycle?.enabled
      ? 'enabled'
      : lifecycle?.approved
  ...
  const badges = [phase, `${plugin.commands} cmd${plugin.commands === 1 ? '' : 's'}`]
...
function marketTab(inventory) {
  const entries = inventory.market.installed.map((plugin) => ({
    ...loadedEntry(plugin),
    status: plugin.lifecycle?.loaded
      ? 'loaded'
...
export async function activate(volund) {
  await volund.commands.register({
    ...
    handler: async (args) => {
      ...
      if (['install', 'inspect', 'approve', 'enable', 'disable', 'uninstall'].includes(action)) {
        if (!target)
