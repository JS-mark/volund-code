/**
 * apollo-plugin-env — 内置插件：/env 斜杠命令，查看 [env] 配置段的生效状态。
 *
 * 数据源是宿主侧 apollo.env.getEffective()（每次调用重读用户级 config.toml 并与
 ...
 *
 * 与 dev 插件同一条装载链路（apollo-sandbox --run-plugin + fd3 桥），
...
export async function activate(apollo) {
  await apollo.commands.register({
    ...
    handler: async () => {
      let entries
      try {
        entries = await apollo.env.getEffective()
      } catch (error) {
      ...
      if (!entries || entries.length === 0)
        return [
          ...
          'Add an [env] section to ~/.apollo/config.toml, for example:',
          '',
          '  [env]',
      ...
      return {
        kind: 'list',
        title: 'Environment — [env] from ~/.apollo/config.toml',
  ...
  await apollo.log.info('apollo-plugin-env activated')
}
