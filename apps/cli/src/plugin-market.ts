...
import { loadTomlFile } from '@apollo-code/config'
import { PluginError, validateManifest, verifyBundle } from '@apollo-code/plugin-runtime'
...
export function parseMarketIndex(value: unknown): MarketIndex {
  if (!isRecord(value) || value.schemaVersion !== 1 || !Array.isArray(value.plugins))
    throw new PluginError('plugin_market_index_invalid', 'expected { schemaVersion: 1, plugins: [] }')
  if (value.plugins.length > MAX_PLUGIN_ENTRIES)
    throw new PluginError('plugin_market_index_invalid', `too many plugins (>${MAX_PLUGIN_ENTRIES})`)
  ...
  for (const raw of value.plugins) {
    ...
    for (const file of entry.files) {
      if (!safeFilePath(file.path) || !DIGEST.test(file.digest))
        throw new PluginError('plugin_market_index_invalid', `unsafe file path/digest in ${entry.name}`)
...
export async function fetchMarketIndex(source: string, signal?: AbortSignal): Promise<MarketIndex> {
  ...
  if (!response.ok)
    throw new PluginError('plugin_market_fetch_failed', `index ${source} responded ${response.status}`)
  const body = await response.text()
  if (body.length > MAX_INDEX_BYTES)
    throw new PluginError('plugin_market_fetch_failed', `index larger than ${MAX_INDEX_BYTES} bytes`)
  try {
...
function fileUrl(source: string, pluginName: string, path: string): string {
  ...
  if (url.origin !== new URL(source).origin)
    throw new PluginError('plugin_market_source_pollution', `file ${path} escaped the market source`)
  return url.toString()
}
...
async function fetchVerifiedFile(
  ...
  if (!response.ok)
    throw new PluginError('plugin_market_fetch_failed', `file ${file.path} responded ${response.status}`)
  const bytes = new Uint8Array(await response.arrayBuffer())
  if (bytes.byteLength > MAX_FILE_BYTES)
    throw new PluginError('plugin_market_fetch_failed', `file ${file.path} larger than ${MAX_FILE_BYTES} bytes`)
...
export async function readMarketMetadata(
  dir: string,
): Promise<MarketInstallMetadata | undefined> {
  let serialized: string
  try {
    serialized = await readFile(join(dir, 'apollo-market.json'), 'utf8')
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code !== 'ENOENT') throw error
    return undefined
  }
  ...
  if (!isRecord(value) || value.schemaVersion !== 1 || !Array.isArray(value.files))
    throw new PluginError('plugin_market_metadata_invalid', `${dir}/apollo-market.json`)
...
/** 装载期完整性映射（apollo-market.json → verifyBundle 的输入形状）。 */
 ...
 * verifyBundle → 写 apollo-market.json → 原子换入 `~/.apollo/plugins/<name>/`。
...
export async function installFromMarket(options: {
  ...
  try {
    ...
    await writeFile(join(staging, 'apollo-market.json'), `${JSON.stringify(metadata, null, 2)}\n`)
...
/** 卸载市场插件：删除 `~/.apollo/plugins/<name>/`（停用由调用方先完成）。 */
export async function uninstallMarketDir(home: string, name: string): Promise<void> {
  ...
  const metadata = await readMarketMetadata(join(marketInstallRoot(home), name)).catch(() => undefined)
  if (!metadata)
...
/** 名字规整：允许裸短名（env → apollo-plugin-env）与全名两种输入。 */
export function normalizePluginName(input: string): string {
  const trimmed = input.trim()
  return PLUGIN_NAME.test(trimmed) ? trimmed : `apollo-plugin-${trimmed}`
}
