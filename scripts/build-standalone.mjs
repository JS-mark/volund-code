...
// apps/cli/bin/apollo.cjs 头注），因此 standalone 实际产出 7 个 target。
...
export async function createNativeManifest(assetDirectory, outputDirectory, target) {
  ...
  for (const kind of ['sandbox', 'search', 'fs']) {
    ...
    try {
      ...
      // 本地 cargo 布局兜底：target/release/apollo-<kind>[.exe] 按目标名收编。
      source = join(assetDirectory, `apollo-${kind}${suffix}`)
      try {
        await access(source)
      } catch {
        throw new Error(
          `missing native asset for ${kind}: expected ${file} or apollo-${kind}${suffix} in ${assetDirectory}`,
        )
      }
    }
...
/**
 * 组装一个 target 的 standalone 目录：<out>/{apollo[.exe], native/, plugins/, checksums.sha256}。
...
export async function buildStandalone({ root, target, assetDirectory, outDirectory }) {
  ...
  const bundle = join(root, 'apps/cli/dist/apollo.js')
  try {
    await access(bundle)
  } catch {
    throw new Error(
      'apps/cli/dist/apollo.js not found; run `pnpm --filter apollo-code build` first',
    )
  }
  ...
  // 运行时解析惯例与 native/ 一致（APOLLO_STANDALONE_ASSET_DIR 或产物旁）。
...
async function main() {
  ...
  const target = process.env.APOLLO_STANDALONE_TARGET ?? hostTarget()
  if (!target)
    throw new Error(
      `unsupported host platform; set APOLLO_STANDALONE_TARGET explicitly (one of: ${VALID_TARGETS.join(', ')})`,
    )

  let assetDirectory = process.env.APOLLO_NATIVE_ASSET_DIR
  if (!assetDirectory) {
    ...
    if (cargo.error ?? cargo.status !== 0)
      throw new Error(
        'cargo build --release failed; install the Rust toolchain or set APOLLO_NATIVE_ASSET_DIR to a directory of prebuilt apollo-<kind>-<target> binaries',
      )
