...
async function tempDir() {
  const dir = await mkdtemp(join(tmpdir(), 'apollo-npm-pack-'))
  dirs.push(dir)
  return dir
}
...
async function fakeStandalone(parent, triple) {
  ...
  const exe = join(dir, isWindows ? 'apollo.exe' : 'apollo')
  ...
  await writeFile(join(dir, 'checksums.sha256'), '0'.repeat(64) + '  apollo\n')
  return dir
}
...
void test('packs meta shell + per-platform packages with npm platform fields', async () => {
  ...
  const darwin = await readJson(join(out, 'apollo-code-darwin-arm64/package.json'))
  assert.equal(darwin.name, '@apollo-code/darwin-arm64')
  ...
  assert.deepEqual(darwin.files, ['apollo', 'native', 'plugins', 'checksums.sha256'])

  const musl = await readJson(join(out, 'apollo-code-linux-x64-musl/package.json'))
  assert.deepEqual(musl.libc, ['musl'])

  // Windows arm64 由 x64 包经 Prism 仿真覆盖：cpu 必须同时声明 arm64，
  // 否则 npm 会在 win32/arm64 上跳过安装，壳解析不到包。
  const windows = await readJson(join(out, 'apollo-code-win32-x64-msvc/package.json'))
  assert.deepEqual(windows.os, ['win32'])
  assert.deepEqual(windows.cpu, ['x64', 'arm64'])
  assert.deepEqual(windows.files, ['apollo.exe', 'native', 'plugins', 'checksums.sha256'])

  const meta = await readJson(join(out, 'apollo-code/package.json'))
  assert.equal(meta.name, 'apollo-code')
  assert.deepEqual(meta.bin, { apollo: 'bin/apollo.cjs' })
  assert.deepEqual(meta.files, ['bin'])
  assert.equal(meta.dependencies, undefined)
  assert.deepEqual(meta.optionalDependencies, {
    '@apollo-code/darwin-arm64': '9.9.9-test',
    '@apollo-code/linux-x64-musl': '9.9.9-test',
    '@apollo-code/win32-x64-msvc': '9.9.9-test',
  })
})
...
void test('thin shell resolves the platform package and forwards argv/exit code', async (t) => {
  ...
  // 模拟 npm 安装后的布局：consumer/node_modules/{apollo-code, @apollo-code/darwin-arm64}
  const consumer = join(fixture, 'consumer')
  await mkdir(join(consumer, 'node_modules', '@apollo-code'), { recursive: true })
  spawnSync('cp', ['-R', join(out, 'apollo-code'), join(consumer, 'node_modules', 'apollo-code')])
  spawnSync('cp', [
    '-R',
    join(out, 'apollo-code-darwin-arm64'),
    join(consumer, 'node_modules', '@apollo-code', 'darwin-arm64'),
  ])

  const run = spawnSync(
    process.execPath,
    [join(consumer, 'node_modules', 'apollo-code', 'bin', 'apollo.cjs'), '--version'],
    { encoding: 'utf8' },
  )
  assert.equal(run.status, 0, run.stderr)
  assert.match(run.stdout, /fake-apollo --version/)
})
