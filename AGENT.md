# AGENT.md — Volund Code 工程约定

> 面向所有在本仓库工作的 Agent（Claude Code / Copilot / Cursor / Codex / 人类）。CLAUDE.md 是本文件的镜像 + Claude 特化补充。

## 1. 项目定位

Volund Code 是 claude-code 的开源平行实现：一个多模型后端的终端 AI 编码 CLI。
不绑定任何 LLM 厂商，Provider 插件化，Rust 承担性能与安全敏感的部分（沙箱、搜索、AST），其余全部 TypeScript + Node ≥ 20。

### 1.1 一等公民架构（不可动摇）

1. **Rust 沙箱是安全基座** —— 所有执行第三方 / 用户 / 未审计代码的路径**默认**走 `volund-sandbox` 独立进程 + 平台原生 syscall 隔离（macOS sandbox-exec / Linux landlock+seccomp / Windows AppContainer）。Bash 工具与 Plugin 宿主**共用**同一套框架（`exec` 模式与 `--run-plugin` 模式），TS 侧禁止自建 sandbox（`node:vm` / `worker_threads` / 直接 spawn）。绕过 = 破坏项目信任模型。详见 §4.7 + spec §5 / §6.4.3。
...
**Type-only 约定**：标注 `[type-only]` 的依赖必须用 `import type { X } from '@volund-code/core'` 形式，tsc 编译时消除，运行时零引入。ESLint 规则 `import/consistent-type-specifier-style` + `import/no-unresolved` 联合把关。
...
- 唯一例外：`packages/auth` 可以 import `@napi-rs/keyring`（属于 native 依赖但不是 volund 自研 crate）。

### 4.4 单入口原则

- 仓库只有 **一个 bin**：`apps/cli`（命令名 `volund`）。
...
- 用户按需装：`volund install-provider openai`，未装的 provider 不进 bundle。
...
- `volund login <provider>` 遵循 **verify-first-store-second**：先调 provider 的最小验证接口（如 `/v1/models`），2xx + body schema 合法**才**写 auth；4xx / 5xx **不落盘**。`--skip-verify` 需 `--dangerous`。
...
- **auth 事件上报（本地 telemetry，见 spec §8.4.1）**：
  ...
  - Sink 默认本地 `~/.volund/telemetry/*.jsonl`（§4.13），**禁止**自动出网。
  - 用途：后期用户可 `volund telemetry export` 导出诊断 / 反馈；OTel opt-in 用户走 §4.13 显式配置上报。

### 4.7 权限与副作用（Rust 沙箱 = 一等公民）

**核心心智**：Rust 沙箱（`volund-sandbox` 独立二进制）是本项目**一等公民架构决策**，与 TS 开发效率同为立项前提。凡涉及执行第三方代码 / 用户命令 / 未审计脚本的路径，**默认**都走 sandbox，不搞例外。这条规则一旦被绕过，就等于**整个信任模型都失效**。
...
| 场景                 | 执行方式                                                 | 一等公民规则                                                            |
|----------------------|----------------------------------------------------------|-------------------------------------------------------------------------|
| **Bash 工具**        | `volund-sandbox exec` 独立进程                           | 不允许 Node 直接 `spawn('/bin/sh')`                                     |
| **Plugin 宿主**      | `volund-sandbox --run-plugin` 独立进程 + JSON-RPC bridge | 不允许 `node:vm` / `worker_threads` / plugin-runtime 内部 `spawn(node)` |
...
- **Skill** = Prompt + 资源。**不执行代码**。放在 `~/.volund/skills/<name>/SKILL.md`，frontmatter 声明 `volundVersion: ^1.0.0`。由 `skills-runtime` 装载。
- **Plugin** = 代码（tools / slash commands / hooks / prompt fragments）。放在 `~/.volund/plugins/<name>/`，必须有 `manifest.json` 声明 `engines.volund: ^1.0.0` + 所需权限。由 `plugin-runtime` 装载。
...
- **JSBridge**：插件唯一入口是 `activate(volund)`。`volund` 对象由 `plugin-host.mjs` 构造为 JSON-RPC 客户端代理，是插件访问 volund 能力的**唯一途径**。禁止扩展 bridge 之外的旁路 API。
- **沙箱（v3 修正后：一等公民 Rust 沙箱）**：
  - 插件**不跑在** volund 主进程内。plugin-runtime 通过 `native-bridge.runPlugin()` 启动 `volund-sandbox --run-plugin` 独立 Node 子进程，走 JSON-RPC 2.0 over NDJSON (fd 3) 与主进程通信。
...
- **权限**：manifest.json 的 `permissions.volund` 是 **RPC method** 白名单（未声明的 method 直接 `-32601 Method not found`）；`permissions.fs/bash/net` 是资源级白名单。所有敏感操作通过 bridge RPC 转发到 `permission` 决策，敏感副作用（`volund.exec`）再套内层 `volund-sandbox exec`。
- **首次装载**：必须弹权限确认对话框，展示 `contributes` + `permissions`。用户批准后写入 `~/.volund/plugins.enabled.toml` 记住权限 hash。
- **升级权限扩容**：任何 `permissions` 变化 → 必须再次弹窗确认。禁止静默继承旧批准。
- **能力边界（红线）**：插件**禁止**通过任何手段：
  - 直接访问 `ProviderClient` / 内部 Router / Runner（`volund.core` 不存在）
  - 直接访问 `native-bridge` / Rust 原生（`volund.native` 不存在）
  - 读写其它插件的 storage / 引用其它插件对象（每插件独立子进程 + 独立 `dataDir`，物理隔离）
  - 修改 `SessionState`（`volund.session` 走 RPC 序列化，天然只读快照）

#### 4.10.1a Provider Plugin 例外（`kind: 'provider'`）

> 详见 spec [PLUGIN-PROVIDER-r1](./docs/superpowers/specs/2026-07-31-volund-code-design/PLUGIN-PROVIDER-r1.md)。普通插件**不能**扩展 Provider；只有 `kind: 'provider'` 特殊插件类别可以，且受最严约束：

- **唯一入口**：`volund.provider.register(spec)` 注册 `ProviderClient` 实现进 `ProviderRegistry`；**不暴露** provider 直调入口（`stream` / `complete` / `getCredential`）—— 调用必经 Router。
...
- **资源守护**：`volund-sandbox` 通过 `setrlimit` 施加 CPU / RSS / NPROC / NOFILE 上限；bridge 调用超时 5s；每 turn 500 次 bridge call 上限；心跳 60s 无响应 → SIGKILL（详见 spec §6.11.2）。
...
- 允许的 contributor（按 priority 默认值）：
  - `builtin` (1000) — volund 自身，`core` 启动自注册
  - `skill:<name>` (800) — `skills-runtime` 激活时注册
  - `project` (600) — `<cwd>/AGENT.md`，`storage` 启动时读取
  - `user` (400) — `~/.volund/PROMPT.md`，`storage` 启动时读取
  - `plugin:<name>` (50) — 插件通过 `volund.prompt.contribute` 注册
...
- 版本策略：volund 主版本升级导致 bridge breaking change 时，plugin-sdk 同步 major，且 volund 支持 semver 兼容多个旧 sdk major。
...
- 找不到时：search/fs worker 降级到 JS fallback；`volund-sandbox` 类 tier=none 并提示 `--dangerous-no-sandbox` 覆盖。
...
- RouterPolicy 从 `ProviderRegistry` 解析 provider 名 → `ProviderClient` 实例；registry 兼纳核心 provider（`provider-anthropic` 等）与插件 provider（`kind:'provider'` 插件注册，见 [PLUGIN-PROVIDER-r1](./docs/superpowers/specs/2026-07-31-volund-code-design/PLUGIN-PROVIDER-r1.md)）。
...
- `telemetry` **默认 sink 是本地文件**：`~/.volund/telemetry/*.jsonl`（对齐 spec §8.1 存储树），**绝不发送任何网络请求**。
- OpenTelemetry / 云上报 sink 属于**显式 opt-in**：只有用户在 `~/.volund/config.toml` 明确写入 `[telemetry.otel] endpoint = "..."` 才启用。
...
- **存储位置固定**：全局 `~/.volund/memory/*.md`，项目 `<cwd>/.volund/memory/*.md`；路径可通过 `[memory].paths.global/project` 覆写，但 **必须** canonicalize + escape 检测，禁止穿越到白名单外。
...
- **200 行 body 上限**：`[memory].max_body_lines` 默认 200。分层降级：
  ...
  2. `memory-runtime.write` 硬校验超限抛错（`VOLUND_MEMORY_TOO_LARGE`）
  3. hook `memory.preWrite` 允许用户/插件放宽或强制拆分
  4. 模型连续 3 次超限 → runtime 自动按 heading 切分保存
  - 修改此限制**必须**同时更新 memory-guide 提示词，否则模型会不自知地反复超限。
- **模型主导**：volund 内置 `memory-guide` 提示词（priority 950）教模型**何时写 / 写什么 / 怎么写**；插件通过 `volund.memory.contributePrompt` 追加片段。禁止在 provider / router / tools 里另起 memory 提示词。
- **召回路径唯一**：模型通过 `volund.memory.recall(query)` 工具主动召回；pinned 条目走 PromptComposer priority 700 自动注入，注入行数受 `[memory].pinned_inject_max_lines` 硬限（默认 400，防炸上下文）。
- **权限门**：所有 `volund.memory.*` bridge 调用**必须**过 `manifest.permissions.memory`（`read` / `write` / `contributePrompt`）声明 + `packages/permission` 决策；未声明的 method 直接 `-32601`。
- **禁止**：
...
- **仅适用于 volund 内部 prompt 装载管线**：PromptComposer contributor（skills / project AGENT.md / user PROMPT.md / plugin fragments / memory md）读入 md 时递归展开。**不作用**于工具 `Read`、`Grep`、用户业务代码。
...
- **双白名单**：路径必须落在 `<cwd>/**` 或 `~/.volund/**` 内（canonicalize 后判定，symlink 逃逸即拒绝）。
...
- **debug 可观测**：`volund debug prompt` dump 出的合并结果里必须保留 `<!-- include: <path> depth=N -->` / `<!-- /include -->` 边界标记。
- **禁止**：
...
- 错误类型继承 `packages/shared` 的 `VolundError`，禁止裸 `throw new Error(...)`。
...
- 设计 spec: `docs/superpowers/specs/2026-07-31-volund-code-design.md`（内容真源）
